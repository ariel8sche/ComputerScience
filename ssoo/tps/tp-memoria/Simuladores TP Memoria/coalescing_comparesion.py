...
# cargar métricas
noC = pd.read_csv("run_best_noC_s0.metrics.csv")
C   = pd.read_csv("run_best_C_s0.metrics.csv")
...
plt.plot(noC['op_idx'], noC['num_free'], label='Sin coalescing', alpha=0.7)
plt.plot(C['op_idx'], C['num_free'], label='Con coalescing', alpha=0.7)
...
plt.figure(figsize=(8,4))
plt.plot(alloc_noC.index, alloc_noC['cum_rate'], label='Sin coalescing', alpha=0.7)
plt.plot(alloc_C.index, alloc_C['cum_rate'], label='Con coalescing', alpha=0.7)
...
plt.figure(figsize=(8,4))
plt.plot(C['op_idx'], C['max_free'], label='Con coalescing', alpha=0.7, )
plt.plot(noC['op_idx'], noC['max_free'], label='Sin coalescing', alpha=0.7)
